...
export async function createFabricProject(): Promise<void> {
    ...
    if (needYo || needGenFab){
        ...
        const installPermission = await window.showQuickPick(['yes','no'],quickPickOptions);
    ...
    if (needYo){
        ...
        try {
            const yoInstOut = await Util.sendCommand('npm install -g yo');
    ...
    if (needGenFab){
        ...
        try {
            const genFabInstOut = await Util.sendCommand('npm install -g generator-fabric');
    ...
    const folderSelect = await window.showOpenDialog(openDialogOptions);
    if (folderSelect){  //undefined if the user cancels the open dialog box
        ...
        const yoFabricCmd = `yo fabric:chaincode -- --language=javascript --name="new-smart-contract" --version=0.0.1 --description="My Smart Contract" --author="John Doe" --license=Apache-2.0 2>&1`;
        try {
